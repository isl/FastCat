...
//////////////// CREATE MODAL WITH MULTIPLE TABLE'S COLUMNS////////////////////////////////////////////////////
/*$("#checkAll").click(function () {
 ...
 $('#json_columns').children().remove();    
 $('#heading_tabels_column').html(templateTitle)
 var html = "";
 
 
 html = html + "<div style='margin: 6px 6px 0px -20px;color: #ffffff;background-color: #347799;'><div style='font-size:15px;text-align: center;'>"+"record_information"+"</div></div>";                               
 for (var k in json.record_information) {
 html = html + "<div class='checkbox'>" +
 "<input class='"+"record_information"+"' type='checkbox' name='checkbox1' id='" + k + "'>" +
 "<label for='" + k + "'>" + k + "</label>" +
 "</div>";
 }
 //json['record_information'] = cat_info;
 ...
 html = html + "<div style='margin: 6px 6px 0px -20px;color: #ffffff;background-color: #347799;'><div style='font-size:15px;text-align: center;'>"+"source_identity"+"</div></div>";
 for (var k in json.source_identity) {
 html = html + "<div class='checkbox'>" +
 "<input class='"+"record_information"+"' type='checkbox' name='checkbox1' id='" + k + "'>" +
 "<label for='" + k + "'>" + k + "</label>" +
 "</div>";
 ...
 //
 // html = html + "<div style='margin: 6px 6px 0px -20px;color: #ffffff;background-color: #347799;'><div style='font-size:15px;text-align: center;'>"+"voyages"+"</div></div>";                               
 //            for (var k in json.voyages[0]) {
 //                html = html + "<div class='checkbox'>" +
 //                        "<input class='"+"voyages"+"' type='checkbox' name='checkbox1' id='" + k + "'>" +
 //                        "<label for='" + k + "'>" + k + "</label>" +
 //                        "</div>";
 //}
 ...
 var lbls = Object.keys(json[key]);
 //// nested table
 if (isFinite(String(lbls[0]))) {            
 var index = Number(lbls[0]);                                                
 if (($(json[key][index][0]).length) > 0) {
 html = html + "<div style='margin: 6px 6px 0px -20px;color: #ffffff;background-color: #347799;'><div style='font-size:15px;text-align: center;'>"+key+"</div></div>";
 
 for (var k in json[key][index][0]) {
 html = html + "<div class='checkbox'>" +
 "<input class='"+key+"' type='checkbox' name='checkbox1' id='" + k + "'>" +
 "<label for='" + k + "'>" + k + "</label>" +
 "</div>";
 }
 }
 ////// simple table
 else if (($(json[key][index]).length) > 0) { 
 console.log(index)
 html = html + "<div style='margin: 6px 6px 0px -20px;color: #ffffff;background-color: #347799;'><div style='font-size:15px;text-align: center;'>"+key+"</div></div>";                               
 for (var k in json[key][index]) {
 html = html + "<div class='checkbox'>" +
 "<input class='"+key+"' type='checkbox' name='checkbox1' id='" + k + "'>" +
 "<label for='" + k + "'>" + k + "</label>" +
 "</div>";
 ...
 $('#json_columns').append(html);
 $("#custom_excel_Modal").modal('show');    
 $('.modal-dialog').drags(); 
 });
 
 ////////////////////////////////////////////////////////////////////////////
 
 $('#custom_excel_cancel').click(function () {
 $("#custom_excel_Modal").modal('hide');
 });
 ...
 var simple_cols = new Array();
 var nested_data = new Array();
 
 $.each($('#json_columns').children(), function (i) {
 if (($(this).children(0)).is(':checked')) {
 values.push(($(this).children(0)).attr('class'));
 keys.push(($(this).children(0)).attr('id'));
 }
 });
 ...
 $.each(uniqueVals, function (i) {
 $.each(json[uniqueVals[i]], function (j) {
 
 var json_values = json[uniqueVals[i]];
 var lbls = Object.keys((json_values[j]));
 
 console.log(lbls)
 if (isFinite(String(lbls[0]))) {
 // Nested               
 $.each((json_values[j]), function (kk) {
 var raw_nested_data = new Object();
 var indexed_vals = new Object();                    
 $.each(keys, function (i) {                       
 if((json_values[j][kk]).hasOwnProperty(keys[i])){
 raw_nested_data[keys[i]] = (json_values[j][kk][keys[i]]);                        
 indexed_vals[j] = raw_nested_data;
 }
 });
 nested_data.push(indexed_vals);
 });
 } else {
 // SIMPLE table
 var raw_data = new Object();
 $.each(keys, function (i) {
 if((json_values[j]).hasOwnProperty(keys[i])){
 raw_data[keys[i]] = ((json_values[j])[keys[i]]);                    
 }
 });
 simple_cols.push(raw_data);
 }
 });
 });
 
 var nested_analysis = new Array();
 if (nested_data.length > 0) {
 
 $.each(nested_data, function (qq) {
 $.each(this, function (cnt) {
 nested_analysis.push(Object.assign({}, (simple_cols[cnt]), nested_data[qq][cnt]));
 });
 });
 } else {
 nested_analysis.push('');
 }
 
 console.log(nested_data)
 console.log(simple_cols)
 
 
 var simple_obj = (Object.assign({},(simple_cols)));  
 var opts = [{sheetid: 'FastCat Record Information', header: true}, {sheetid: 'Selected Nested', header: true}];
 //var res = alasql('SELECT * INTO XLSX("Custom-' + $('#unique_filename').text() + '.xlsx",?) FROM ?', [opts, [simple_obj, nested_analysis]]);             
 $("#custom_excel_Modal").modal('hide');  
 });
